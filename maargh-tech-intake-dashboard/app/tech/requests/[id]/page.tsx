...
export default function RequestDetail(){
 ...
 useEffect(()=>{
  ...
  ]).then(([request,events])=>{setItem(request.data);setActivity(events.data??[])})
 },[supabase,id])
 ...
 async function addComment(e:React.FormEvent){
  ...
  await supabase.from('tech_request_activity').insert({request_id:id,author_name:'Tech team',body:comment.trim()})
  setComment('')
  const {data}=await supabase.from('tech_request_activity').select('*').eq('request_id',id).order('created_at',{ascending:false})
  setActivity(data??[])
 }
 ...
 return <main className="detail-page">
  ...
  <header className="detail-header"><div><span className="eyebrow">{item.request_code}</span><h1>{item.title}</h1><p>{item.requester_name} · {item.requester_email} · Submitted {new Date(item.created_at).toLocaleString()}</p></div><button className="primary-button" onClick={save} disabled={busy}><Save size={16}/> {busy?'Saving…':'Save changes'}</button></header>
